# Packet S7b (supersedes S7: schema added to scope after an honest stop) — classify `.go` files; register the Sol xHigh route so a Codex supervisor can attest

- Kind: `impl`; Class: `impl/deterministic/data-schema/s/python`; Declared size: 6 files, ≤ 200 lines
- Owned paths: `config/staffing/classes.yaml`, `config/staffing/schema/classes.schema.json` (the language value set and the class-block language enum only), `src/lee_llm_router/staffing/derive_class.py`,
...
- Runtime bound: 20 minutes. Oracle (fails before the change): `bash -c "python3 -m pytest -q tests/test_staffing_derive_class.py tests/test_staffing_catalog.py tests/test_staffing_catalog_explain.py && lee-llm-router route show codex-gpt-5-6-sol-xhigh-openai-sub && lee-llm-router staff --mode auto --role impl --class impl/deterministic/none/s/go --json"`
